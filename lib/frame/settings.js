...
Http2SettingsFrame.prototype.toString = function () {
    ...
    if (this.getParamCount()) {
        ...
        for (var i = 0; i < this.getParamCount(); i++) {
            ...
            } else if (param.id === Http2SettingsFrame.PARAM_SETTINGS_MAX_FRAME_SIZE) {
                params.push('  SETTINGS_MAX_FRAME_SIZE: ' + param.value);
            } else if (param.id === Http2SettingsFrame.PARAM_SETTINGS_MAX_HEADER_LIST_SIZE) {
                params.push('  SETTINGS_MAX_HEADER_LIST_SIZE: ' + param.value);
            } else {
