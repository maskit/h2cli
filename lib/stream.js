        ...
        if (f.streamId > 0) {
            switch (self.state) {
                    ...
                    switch (f.type) {
                        case h2frame.Http2HeadersFrame.TYPE_CODE:
                            if (f.flags & h2frame.Http2HeadersFrame.FLAG_END_STREAM) {
                                self.state = Stream.STATE_HALF_CLOSED_LOCAL;
                            } else {
                                self.state = Stream.STATE_OPEN;
                            }
                            break;
                        case h2frame.Http2PushPromiseFrame.TYPE_CODE:
                            self.state = Stream.STATE_RESERVED_LOCAL;
                            break;
                        default:
                            break;
                    }
                ...
                case Stream.STATE_RESERVED_LOCAL:
                    switch (f.type) {
                        case h2frame.Http2HeadersFrame.TYPE_CODE:
                            self.state = Stream.STATE_HALF_CLOSED_REMOTE;
                            break;
                        case h2frame.Http2RstStreamFrame.TYPE_CODE:
                            self.state = Stream.STATE_CLOSED;
                            break;
                        default:
                            break;
                    }
    ...
    if (frame.streamId > 0) {
        switch (this.state) {
            case Stream.STATE_IDLE:
                switch (frame.type) {
                    case h2frame.Http2HeadersFrame.TYPE_CODE:
                        this.state = Stream.STATE_OPEN;
                        break;
                    case h2frame.Http2PushPromiseFrame.TYPE_CODE:
                        this.state = Stream.STATE_RESERVED_REMOTE;
                        break;
                    default:
                        break;
                }
            ...
            case Stream.STATE_RESERVED_LOCAL:
                switch (frame.type) {
                    case h2frame.Http2RstStreamFrame.TYPE_CODE:
                        this.state = Stream.STATE_CLOSED;
                        break;
                    case h2frame.Http2PriorityFrame.TYPE_CODE:
                        // do nothing
                        break;
            ...
            case Stream.STATE_RESERVED_REMOTE:
                switch (frame.type) {
                    case h2frame.Http2HeadersFrame.TYPE_CODE:
                        this.state = Stream.STATE_HALF_CLOSED_LOCAL;
                        break;
                    case h2frame.Http2RstStreamFrame.TYPE_CODE:
                        this.state = Stream.STATE_CLOSED;
                        break;
            ...
            case Stream.STATE_OPEN:
                if (frame.constructor.FLAG_END_STREAM && frame.flags & frame.constructor.FLAG_END_STREAM) {
                    this.state = Stream.STATE_HALF_CLOSED_REMOTE;
