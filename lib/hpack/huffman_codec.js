...
table.forEach(function (entry, i) {
    if (!decodeHints[entry.len]) {
        decodeHints[entry.len] = [];
    }
    decodeHints[entry.len].push([entry.value, i]);
});
...
module.exports.decode = function (data, offset, length) {
    ...
    var output = length > 1024 ? new Buffer(length * 4) : tmp,
        len = 0;
    for (i = 0; i < length; ++i) {
        for (j = 0x80; j > 0; j = j >>> 1) {
            ...
            if (decodeHints[n]) {
               if (decodeHints[n][0][0] <= code && decodeHints[n][decodeHints[n].length - 1][0] >= code) {
