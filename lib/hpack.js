...
HPACK.prototype.decode = function (input) {
    ...
    while (pos < input.length) {
        ...
        if (result.type === REPRESENTATION_INDEXED) {
            ...
            if (result.index <= this.lHeaderTable.length) {
                ...
                ref = this.lHeaderTable.insert(field[NAME], field[VALUE]);
                if (ref) {
    ...
    for (ref in this.lReferenceSet) {
        ...
        for (i = 0; i < output.length; i++) {
            if (output[i][NAME] === field[NAME] && output[i][VALUE] === field[VALUE]) {
                break;
            }
        }
        if (i === output.length) {
            // console.log('HPACK-DEBUG: Append ' + field[NAME] + ': ' + field[VALUE] + ' from RefSet');
            output2.push([field[NAME], field[VALUE]]);
        }
    }
