...
var createFieldGetter = function (definition) {
    switch (definition.type) {
        ...
        case 'BUFFER':
            return function () {
                if (definition.byteLength) {
                    return this.buf.slice(definition.offset, definition.offset + definition.byteLength);
                } else {
                    return this.buf.slice(definition.offset);
                }
            };
        default:
            return function () {
...
var createFieldSetter = function (definition) {
    switch (definition.type) {
        ...
        case 'BUFFER':
            return function (value) {
                if (definition.byteLength) {
                    value.copy(this.buf, definition.offset, 0, definition.byteLength);
                } else {
                    this.buf = Buffer.concat([this.buf.slice(0, definition.offset), value]);
                }
            };
        default:
            return function (value) {
...
Http2Frame.defineType = function(frame, definitions) {
    Object.defineProperty(frame, 'TYPE_NAME', {
        get: function () {
            return definitions.NAME;
        }
    });
    Object.defineProperty(frame, 'TYPE_CODE', {
        get: function () {
...
Http2Frame.defineFields(Http2Frame, {
    length:    { type: 'UINT', offset: OFFSET_LENGTH_FIELD,   bitLength: 3 * 8 },
    type:      { type: 'UINT', offset: OFFSET_TYPE_FIELD,     bitLength: 1 * 8 },
    flags:     { type: 'UINT', offset: OFFSET_FLAGS_FIELD,    bitLength: 1 * 8 },
    streamId:  { type: 'UINT', offset: OFFSET_STREAMID_FIELD, bitLength: 31 },
});
...
Http2Frame.prototype.toString = function () {
    ...
    str += "Type: " + this.constructor.TYPE_NAME + "(" + this.type + "), ";
