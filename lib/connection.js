...
Http2Reader.prototype._read = function(n) {
    ...
    do {
        if (this._fsize === -1) {
            this._fsize = (this._bufArray[0].readUInt16BE(0) & 0x3FFF) + 8;
        }
    ...
    } while (this._bufArray[0].length >= 2);
    this.push('');
};
...
var Connection = module.exports.Connection = function (hostname, port, options) {
    ...
    if (this.secure) {
        ...
        if (options['useNpn']) {
            opt['NPNProtocols'] = ['h2-13'];
        }
        this.stream = require('tls').connect(opt);
