...
var Connection = module.exports.Connection = function (hostname, port, options) {
    ...
    this.hpack = new h2hpack(this._localSettings[1], this._remoteSettings[1]);
    ...
    if (options.useUpgrade) {
        this.stream.on('readable', function () {
            var chunk = this.read();
            var response = chunk.toString('ascii');
            ...
            if (end == -1) {
                ...
                this.unshift(chunk.slice(end + 4));
            }
            self.reader = new Http2Reader(self.stream);
            self.reader.on('frame', function (frame) {
    ...
    this.stream.on('end', function (had_error) {
    });
};
